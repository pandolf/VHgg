...
RedNtpFinalizer_TTVHgg::~RedNtpFinalizer_TTVHgg()
{
...
void RedNtpFinalizer_TTVHgg::finalize()
{
   ...
   // this is the uncorrelated BSM search category:
   bool isBSMEvent_t;
	...
	idphot2 = (idcicpfphot2 < photonID_thresh_);
	 ...
	 for( unsigned j=i+1; j<index_selected.size(); ++j ) {
       ...
       // *****   (2 jets, 0 btag loose)
         ...
         if( (ebeb!=ebeb_0btag_thresh_) ) continue;
...
Int_t RedNtpFinalizer_TTVHgg::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!tree_) return 0;
   return tree_->GetEntry(entry);
}
Long64_t RedNtpFinalizer_TTVHgg::LoadTree(Long64_t entry)
{
...
void RedNtpFinalizer_TTVHgg::Init()
{
...
void RedNtpFinalizer_TTVHgg::setSelectionType( const std::string& selectionType ) {
