...
./combine -M HybridNew --frequentist datacard.root --expectedFromGrid=0.5 --saveToys --fullBToys --saveHybridResult -T 200 -t$n -s$1 -H ProfileLikelihood >& log.txt
mv higgs*.root outputToy/
mv log.txt outputToy/
echo "pack the results"
tar cvf outputToy.tar outputToy/
